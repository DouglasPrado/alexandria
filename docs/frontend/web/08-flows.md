...
7. Membro clica em arquivo → modal lightbox abre com preview ampliado
   ...
   - PDF: leitor completo via `PDFViewer` (react-pdf) — navegação por páginas, zoom, text layer, download
   - Documento (não-PDF): sem preview (exibe ícone por extensão + metadados no cliente)
...
- Remoção deixaria cluster com <1 nó → Modal bloqueado: "Não é possível remover — mínimo de 1 nó necessário"
