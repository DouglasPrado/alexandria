...
async function fetchWithMetrics(url: string, options: RequestInit) {
  ...
  try {
    ...
    Sentry.metrics.distribution('api.duration', duration, { unit: 'millisecond', tags: { endpoint: url, status: String(response.status) } });

    if (!response.ok && response.status >= 500) {
      Sentry.metrics.increment('api.error', 1, { tags: { endpoint: url, status: String(response.status) } });
    }
