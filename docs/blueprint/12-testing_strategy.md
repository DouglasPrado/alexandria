...
| **Ferramentas** | `cargo test` (built-in Rust); `proptest` para property-based testing de crypto; `criterion` para benchmarks |
...
> **Integrações críticas:** SQLx ↔ PostgreSQL (transações atômicas), StorageProvider ↔ S3 (put/get chunks), FFmpeg (transcodificação), Recovery (seed → vault → rebuild)
...
| **Escopo — O que testar** | UC-001: Criar cluster (seed phrase gerada, vault criado); UC-004: Upload de foto (aparece na galeria com thumbnail); UC-005: Download de arquivo (descriptografa e reassembla); UC-007: Recovery via seed (nova VPS → sistema operacional); Auto-healing: remover nó → chunks re-replicados |
| **Ferramentas** | `cargo test` com setup Docker Compose completo (orquestrador + pg + redis + minio); Playwright para testes do Web Client (Next.js 16); scripts shell para disaster drill automatizado |
...
| **Ferramentas** | `k6` para load testing HTTP; `criterion` para microbenchmarks Rust (crypto, hashing); `pgbench` para queries PostgreSQL |
...
| **Ferramentas** | Scripts shell/Rust para simular falhas; `docker stop/kill` para parar containers; `dd` para corromper chunks; disaster drill manual mensal |
...
| Integration Tests | >60% | Cobre contratos críticos: SQLx queries, StorageProvider, pipeline de mídia |
...
|---|---|---|
| Local (cargo test) | Desenvolvimento: unit tests rápidos + clippy + fmt | Fixtures embarcadas no código; crypto com chaves de teste |
| CI (GitHub Actions) | Unit + integration automatizados a cada PR/push | testcontainers (PostgreSQL, Redis, MinIO em Docker); fotos/vídeos sample (~10MB total) |
...
|---|---|---|---|
| Pull Request | cargo test (unit) + cargo clippy + cargo fmt --check | Push / abertura de PR | Sim |
| Merge na main | cargo test (unit + integration com testcontainers) | Merge | Sim |
...
> **Tempo máximo do pipeline CI (PR → merge):** <10 min (unit + clippy + fmt: ~3 min; integration: ~5 min)
