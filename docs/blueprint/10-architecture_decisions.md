...
**Decisão:** Hierarquia: seed (12 palavras BIP-39) → master key (PBKDF2) → file keys (derivação) → chunk keys (AES-256-GCM). Vault criptografado individual por membro (cada membro tem seu próprio vault desbloqueado com sua senha) para tokens/credenciais/senhas. Master key nunca persistida; usada apenas em recovery do orquestrador.
